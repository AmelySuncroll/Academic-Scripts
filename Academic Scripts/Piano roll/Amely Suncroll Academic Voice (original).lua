...
-- @changelog
--    + init @
-- @about Select any voice in selected tracks as original academic symbols: 1 = soprano + notes without chords, 2 = alto, 3 = tenor, 4 = bass, etc. 0 = all lowest notes inside chords. 
...
function select_voice_notes(voice_num, startPositions, notes)
    ...
    for _, indices in pairs(startPositions) do
        if #indices > 1 then
            table.sort(indices, function(a, b) return notes[a][3] > notes[b][3] end) -- Sort by pitch (descending)
            if voice_num == 0 then

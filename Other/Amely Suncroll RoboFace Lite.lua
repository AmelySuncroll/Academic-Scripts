...
--    + 1.24 add lite version - pause all functions when a left, right or middle mouse click is hold
...
gfx.init("RoboFace 1.24 Lite", startWidth, startHeight, dock_state, x, y)
...
function get_script_window_position()
  local hwnd = reaper.JS_Window_Find("RoboFace 1.24 Lite", true)
  local retval, left, top, right, bottom = reaper.JS_Window_GetRect(hwnd)
  local width = right - left
  local height = bottom - top
...
function welcome_message()
    if current_language == "en" then
        ...
        -- reaper.ShowConsoleMsg("RoboFace 1.24 Lite\n")
    else
...
function ShowMenu(menu_str, x, y)
    ...
    if is_windows or is_macos and is_full_screen then
        ...
        gfx.init('RoboFace 1.24 Lite', 0, 0, 0, x, y)
...
function show_r_click_menu()
    ...
    local script_hwnd = reaper.JS_Window_Find("RoboFace 1.24 Lite", true)

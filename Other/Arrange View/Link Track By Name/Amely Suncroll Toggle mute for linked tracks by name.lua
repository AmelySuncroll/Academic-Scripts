--[[
  ...
  ReaScript Name: Toggle mute for linked track by name
...
local function Exit()
    is_running = false
    reaper.ShowMessageBox("Script stopped.", "Toggle MUTE for linked tracks", 0)
end
...
local function monitor_and_sync_mute_states()
    ...
    for _, pair in ipairs(linked_tracks) do
        ...
        if is_muted1 ~= previous_mute_state[pair_id][1] or is_muted2 ~= previous_mute_state[pair_id][2] then
            if is_muted1 ~= is_muted2 then
                if is_muted1 ~= previous_mute_state[pair_id][1] then
                    reaper.SetMediaTrackInfo_Value(track2, "B_MUTE", is_muted1 and 1 or 0)
                else
                    reaper.SetMediaTrackInfo_Value(track1, "B_MUTE", is_muted2 and 1 or 0)
                end
            end
...
local function main()
    ...
    if not is_running then
        ...
        reaper.ShowMessageBox("Script working", "Toggle MUTE for linked tracks", 0)
    else
        Exit()
    end
end

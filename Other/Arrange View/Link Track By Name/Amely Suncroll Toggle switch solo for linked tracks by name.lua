--[[
  ...
  ReaScript Name: Toggle switch solo for linked track by name
...
local function Exit()
  is_running = false
  reaper.ShowMessageBox("Script stopped", "Switch SOLO for linked tracks", 0)
end
...
local function monitor_and_sync_mute_states()
    ...
    for _, pair in ipairs(linked_tracks) do
        ...
        local is_muted1 = reaper.GetMediaTrackInfo_Value(track1, "I_SOLO") > 0
        local is_muted2 = reaper.GetMediaTrackInfo_Value(track2, "I_SOLO") > 0
        ...
        if is_muted1 ~= previous_mute_state[pair_id][1] or is_muted2 ~= previous_mute_state[pair_id][2] then
            if is_muted1 ~= previous_mute_state[pair_id][1] then
                reaper.SetMediaTrackInfo_Value(track2, "I_SOLO", is_muted1 and 0 or 1)
            else
                reaper.SetMediaTrackInfo_Value(track1, "I_SOLO", is_muted2 and 0 or 1)
            end

            previous_mute_state[pair_id] = {reaper.GetMediaTrackInfo_Value(track1, "I_SOLO") > 0, reaper.GetMediaTrackInfo_Value(track2, "I_SOLO") > 0}
        end
    end
...
local function main()
    ...
    if not is_running then
        ...
        reaper.ShowMessageBox("Script working", "Switch SOLO for linked tracks", 0)
    else
        Exit()
    end
end
